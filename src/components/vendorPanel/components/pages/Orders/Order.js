...
const Order = () => {
  ...
  const fetchData = async () => {
    try {
      const { data } = await axios.get(
        "https://6nkuu0ytgg.execute-api.ap-south-1.amazonaws.com/dev//api/v1/admin/orders",
        {
          headers: {
  ...
  const postHandler = async (id) => {
    try {
      const { data } = await axios.post(
        `https://6nkuu0ytgg.execute-api.ap-south-1.amazonaws.com/dev//api/v1/user/paymentstatus/${id}`
      );
      console.log(data);
      fetchData();
    } catch (e) {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const assingOrder = async (e) => {
      ...
      try {
        const { data } = await axios.post(
          "https://6nkuu0ytgg.execute-api.ap-south-1.amazonaws.com/dev//api/v1/driver/addOrder ",
          {
            orderId,
            driverId,
          }
